...
internals.htmlifyError = function htmlifyError (err) {
  var lines = (new AnsiToHTML({escapeXML: true})).toHtml(err).split('\n')
  ...
  return '<div style="font-family: \'helvetica-neue\', helvetica">'
    + html
    + '</div>'
    + '</body></html>'
}
...
internals.serveDefaultPage = function serveDefaultPage (req, res, args) {
  internals.getCustomDefaultHTML(req, args, function gotCustomDefaultHTML (err, html) {
    ...
    if (err) {
      prettyifiedErr = prettyError(err)
      bs.logger.error(prettyifiedErr)
      res.statusCode = 500
      res.end(internals.htmlifyError(prettyifiedErr))
    }
    else {
      res.end(src)
    }
...
internals.responder = function responder (type, res) {
  return function response (err, src) {
    ...
    if (err) {
      prettyifiedErr = prettyError(err)
      bs.logger.error(type + ': ' + prettyifiedErr)

      if (!res.headersSent) res.setHeader('Content-Type', 'text/html')
      res.statusCode = 500
      return void res.end(internals.htmlifyError(prettyifiedErr))
    }
