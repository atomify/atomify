...
module.exports = function (args) {
  ...
  if (lr) startLiveReloadServer(lr)
  ...
  http.createServer(function (req, res) {
    ...
    switch (req.url) {
      ...
      default:
        if (req.url.substr(-5) === '.html') res.filter = injectLiveReloadScript()
        mount(req, res)
        break
    }
...
function injectLiveReloadScript () {
  var buffer = ''
    , tag = '<script src="http://localhost:35729/livereload.js?snipver=1"></script>\n'
