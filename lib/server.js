...
internals.htmlifyBabelError = function htmlifyBabelError (err) {
...
internals.htmlifyError = function htmlifyError (err) {
  ...
  if (err._babel) return internals.htmlifyBabelError(err)

  lines = (new AnsiToHTML({escapeXML: true})).toHtml(err).split('\n')
  html = '<!doctype html><html><head>'
    + '<meta charset="utf-8">'
    + '<meta http-equiv="X-UA-Compatible" content="IE=edge">'
    + '<meta name="viewport" content="initial-scale=1,width=device-width,user-scalable=0">'
...
internals.sendError = function sendError (err, res) {
  ...
  try {
    res.end(internals.htmlifyError(prettyifiedErr || err))
  }
  catch (e){
